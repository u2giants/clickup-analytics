...
- **A bespoke front-end is a Phase-2 polish layer, built incrementally per role** — only for the adoption-critical roles (sales, designers) if Data Studio's admin-shaped UX isn't friendly enough. When we build it:
  - **One external React app**, assembled from open-source, using an admin/CRUD framework that gives the AI a Directus data provider + auth + CRUD so it writes mostly screens, not plumbing. **Use `react-admin` + `ra-directus`** (officially maintained by Marmelab, last release 2025) — **not Refine**, whose Directus connector (`@tspvivek/refine-directus`) is community-only and near-dormant (no release in ~2 years). *(Caveat: `ra-directus`'s built-in login is local email/password; SSO is handled by Directus at the backend and the app rides the resulting session — see SSO note.)*
  - Open-source building blocks: `@directus/sdk`, react-admin (or Refine if you accept the dormant connector), **shadcn/ui** + Tailwind, **TanStack Table** (grids), **dnd-kit** (kanban), **FullCalendar** (calendar/gantt), React Hook Form + Zod (forms), Recharts/ECharts (dashboards).
...
**SSO — resolved (research 2026-06-09):** **Microsoft Entra and Google are config-only** on Directus (OpenID driver, env vars; works on Core+OIG). **WeChat is NOT** — WeChat's OAuth is non-standard (`appid`/`secret`, QR flow, no email) *and* Directus does not allow custom auth-driver extensions, so WeChat SSO would require forking core or building a side service that mints Directus sessions. **Action: confirm whether WeChat SSO is actually required** (e.g., for the China team); if it is, budget custom dev or use Microsoft/Google + manual accounts for those users.
...
> **✅ Phase-1 backend BUILT & VERIFIED (2026-06-09)** — see [`../pm-system/`](../pm-system/). 14 collections (POP 2-tier + Spruce 3-tier), 26 relations, the Designer field-level pricing-hide policy, and the stage-history Flow are implemented as a deployable Directus project (`apply-schema.mjs` + `docker-compose.yml`), with all three core proofs passing (`seed-and-verify.mjs`). Built/tested locally; ready to deploy on Coolify with the OIG license key. Remaining Phase-1.x: M2M relations, the other Flows, ClickUp→Directus migration import.
